...
class Trainer_fmri_text:
    ...
    def train(self, local_rank):
        ...
        for epoch in progress_bar:
            ...
            self.loss_mae_text_sum = 0.
            self.loss_nce_text_sum = 0.
            ...
            self.val_loss_mae_text_sum = 0.
            self.val_loss_nce_text_sum = 0.
    ...
    def train_step(self, voxel, captions, subj_id, epoch):
        # def train_step(self, voxel, image, captions, subj_id, epoch):
        loss = 0.
        ...
        if self.args.mse_mult:
            loss_mse_text = nn.MSELoss()(fmri_text, clip_text)
        ...
        if self.args.mae_mult:
            loss_mae_text = nn.L1Loss()(fmri_text, clip_text)
            utils.check_loss(loss_mae_text, "loss_mae_text")
            loss += self.args.mae_mult * loss_mae_text
            self.loss_mae_text_sum += loss_mae_text.item()

        # text infonce loss
        if self.args.nce_mult:
            loss_nce_text = utils.info_nce(fmri_text, clip_text, 0.1)
            loss += self.args.info_nce_mult * loss_nce_text
            self.loss_nce_text_sum += loss_nce_text.item()
        ...
        self.sims_text += nn.functional.cosine_similarity(fmri_text, clip_text).mean().item()

    @abstractmethod
    def eval_epoch(self, epoch):
        pass

    def eval_step(self, voxel, captions, subj_id, epoch):
        # def eval_step(self, voxel, image, captions, subj_id, epoch):
        val_loss = 0.
        with torch.no_grad():
            # get clip text embedding and fmri results
            clip_text = self.clip_extractor.embed_text(captions).flatten(1)
            ...
            # text mse loss
            if self.args.mse_mult:
                val_loss_mse_text = nn.MSELoss()(fmri_text, clip_text)
                utils.check_loss(val_loss_mse_text, "loss_mse_text")
                val_loss += self.args.mse_mult * val_loss_mse_text
                self.val_loss_mse_text_sum += val_loss_mse_text.item()

            # text mae loss
            if self.args.mae_mult:
                val_loss_mae_text = nn.L1Loss()(fmri_text, clip_text)
                utils.check_loss(val_loss_mae_text, "loss_mae_text")
                val_loss += self.args.mae_mult * val_loss_mae_text
                self.val_loss_mae_text_sum += val_loss_mae_text.item()

            # text infonce loss
            if self.args.nce_mult:
                val_loss_nce_text = utils.info_nce(fmri_text, clip_text, 0.1)
                val_loss += self.args.info_nce_mult * val_loss_nce_text
                self.val_loss_nce_text_sum += val_loss_nce_text.item()
            ...
            self.val_sims_text += nn.functional.cosine_similarity(fmri_text, clip_text).mean().item()
    ...
    def save(self, epoch):
        ...
        # save best model
        current_sim = (self.val_sims_text) / (self.val_i + 1)
    ...
    def log_train(self):
        self.logs = {
            ...
            "train/loss_nce_text": self.loss_nce_text_sum / (self.train_i + 1),
            "train/loss_mse_text": self.loss_mse_text_sum / (self.train_i + 1),
            "train/loss_mae_text": self.loss_mae_text_sum / (self.train_i + 1),
        ...
        print(f"train/cosine_sim_text: {self.sims_text / (self.train_i + 1)}")
        print(f"train/loss_nce_text: {self.loss_nce_text_sum / (self.train_i + 1)}")
        print(f"train/loss_mse_text: {self.loss_mse_text_sum / (self.train_i + 1)}")
        print(f"train/loss_mae_text: {self.loss_mae_text_sum / (self.train_i + 1)}")
    ...
    def log_val(self):
        self.logs.update(
            {
                ...
                "val/loss_nce_text": self.val_loss_nce_text_sum / (self.val_i + 1),
                "val/loss_mse_text": self.val_loss_mse_text_sum / (self.val_i + 1),
                "val/loss_mae_text": self.val_loss_mae_text_sum / (self.val_i + 1),
        ...
        print(f"val/cosine_sim_text: {self.val_sims_text / (self.val_i + 1)}")
        print(f"val/loss_nce_text: {self.val_loss_nce_text_sum / (self.val_i + 1)}")
        print(f"val/loss_mse_text: {self.val_loss_mse_text_sum / (self.val_i + 1)}")
        print(f"val/loss_mae_text: {self.val_loss_mae_text_sum / (self.val_i + 1)}")
        print(f"val/loss_rec: {self.val_loss_rec_sum / (self.val_i + 1)}")
        print(f"val/loss_cyc: {self.val_loss_cyc_sum / (self.val_i + 1)}")


class Trainer_fmri_text_single(Trainer_fmri_text):
    def __init__(self, args, accelerator, voxel2clip, clip_extractor, prompts_list, device) -> None:
    ...
    def prepare_dataloader(self):
        ...
        self.train_dl, self.val_dl = data.get_dls(
            ...
            extensions = ['nsdgeneral.npy', 'coco73k.npy', 'subj'],
...
class Trainer_fmri_text_bridge(Trainer_fmri_text):
    def __init__(self, args, accelerator, voxel2clip, clip_extractor, prompts_list, device) -> None:
        super().__init__(args, accelerator, voxel2clip, clip_extractor, prompts_list, device)
    ...
    def train_epoch(self, epoch):
        ...
        for train_i, datas in enumerate(zip(*self.train_dls)):
            ...
            for voxel, coco, subj_id in datas:
...
class Trainer_fmri_text_adapt(Trainer_fmri_text):
    def __init__(self, args, accelerator, voxel2clip, clip_extractor, prompts_list, device) -> None:
        super().__init__(args, accelerator, voxel2clip, clip_extractor, prompts_list, device)
