...
class Trainer_fmri_image:
    ...
    def train_step(self, voxel, image, subj_id, epoch):
        ...
        clip_image = self.clip_extractor.embed_image_with_DIVA(image).flatten(1)
        fmri_image, fmri_rec, loss_cyc = self.voxel2clip((voxel, subj_id))

        # norm clip_image and fmri_image
        clip_image = nn.functional.normalize(clip_image, dim = -1).float()
        fmri_image = nn.functional.normalize(fmri_image, dim = -1).float()
        ...
        if self.args.info_nce_mult:
            loss_nce_image = utils.info_nce(fmri_image, clip_image, 0.1)
